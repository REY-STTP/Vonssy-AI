            ...
            {renamingId === session.id ? (
              ...
              >
                <div className="flex items-center gap-1.5">
                  {session.isPinned && (
                    <span className="text-accent text-xs" title="Pinned">★</span>
                  )}
                  <span
            ...
            {renamingId !== session.id && (
              <div className="absolute right-2 top-2" ref={menuOpenId === session.id ? menuRef : null}>
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 focus:opacity-100 text-text-secondary hover:text-text-primary px-1 py-0.5 text-sm transition-opacity"
                ...
                {menuOpenId === session.id && (
                  <div className="absolute right-0 top-6 z-50 bg-surface border border-border rounded-lg shadow-soft min-w-[140px] overflow-hidden">
                    <button type="button" onClick={() => startRename(session)} className="w-full text-left px-3 py-2 text-xs font-medium text-text-primary hover:bg-surface-raised transition-colors">
                      Rename
                    </button>
                    <button
                      type="button"
                      onClick={() => { onTogglePin(session.id, !session.isPinned); setMenuOpenId(null); }}
                      className="w-full text-left px-3 py-2 text-xs font-medium text-text-primary hover:bg-surface-raised transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-xs font-medium text-danger hover:bg-surface-raised transition-colors border-t border-border"
                    >
                      Delete
                    </button>
                  </div>
      ...
      <div className="border-t border-border p-3 space-y-3">
        {user && (
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={onOpenSettings} className="p-1.5 text-text-secondary hover:text-text-primary hover:bg-surface-raised rounded-md transition-colors" aria-label="Settings" title="Settings">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <button type="button" onClick={() => signOut()} className="p-1.5 text-text-secondary hover:text-danger hover:bg-surface-raised rounded-md transition-colors" aria-label="Sign out" title="Sign Out">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  const collapsedContent = (
    <div className="flex flex-col items-center h-full bg-surface border-r border-border py-3 gap-2">
      ...
      <button
        type="button"
        onClick={onNewChat}
        className="flex items-center justify-center w-10 h-10 rounded-lg bg-accent text-accent-contrast hover:opacity-90 transition-opacity"
        aria-label="New Chat"
        title="New Chat"
      >
