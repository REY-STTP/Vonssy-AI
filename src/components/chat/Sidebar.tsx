...
interface SidebarProps {
  ...
  /* ── Collapse Toggle Icon (hoisted: stable identity) ─── */
...
// D4: hoisted so date math isn't re-created per render.
...
function Sidebar({
  ...
  // G1: mobile drawer dialog — focus panel on open, Escape closes,
  ...
  useEffect(() => {
    ...
    mobileReturnFocusRef.current = document.activeElement as HTMLElement;
    const timer = setTimeout(() => mobilePanelRef.current?.focus(), 50);
  ...
  const expandedContent = (
    <div className="flex flex-col h-full bg-surface border-r border-border">
      ...
      {/* Session list */}
      <nav aria-label={t("sidebar.menu")} className="flex-1 overflow-y-auto p-2 space-y-1">
        {sessions.length === 0 && (
            ...
            {renamingId !== session.id && (
              <div className="absolute right-2 top-2" ref={menuOpenId === session.id ? menuRef : null}>
                <button
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Escape" && menuOpenId === session.id) {
                      setMenuOpenId(null);
                    }
                  }}
                >
                ...
                {menuOpenId === session.id && (
                  ...
                  >
                    <button type="button" role="menuitem" onClick={() => startRename(session)} className="w-full text-left px-3 py-2.5 text-xs font-medium text-text-primary hover:bg-surface-raised transition-colors flex items-center gap-2">
      ...
      {isOpen && (
        <>
          ...
          <aside
            ...
            tabIndex={-1}
            className="md:hidden fixed inset-y-0 left-0 w-[280px] z-50 shadow-[0_8px_32px_rgba(0,0,0,0.16)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5)] animate-modal-enter focus:outline-none"
          >
            {expandedContent}
          </aside>
        </>
